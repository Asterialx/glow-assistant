  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 rounded-full bg-[var(--bg-hover)] px-2.5 py-1 text-[12.5px] hover:bg-[var(--bg-active)]"
      >
        <span className="font-semibold text-[var(--fg)]">{selected.displayName}</span>
        <ChevronDown size={13} className="opacity-60" />
      </button>

      {open && (
        <div className="absolute bottom-full right-0 z-50 mb-2 max-h-[min(480px,55vh)] w-[340px] overflow-y-auto rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] py-1 shadow-2xl">
          {list.map((m) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full items-start gap-2 px-3.5 py-2.5 text-left hover:bg-[var(--bg-hover)]",
                  active && "bg-[var(--bg)]",
                )}
              >
          ...
          <div className="mt-1 space-y-3 border-t border-[var(--border)] px-3.5 py-3">
            ...
            <button
              ...
              disabled={!dirty}
              className={cn(
                "flex w-full items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] px-3 py-2 text-[12.5px] font-medium transition-colors",
                dirty
